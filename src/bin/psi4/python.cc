...
std::vector<SharedMatrix> mixed_atomic_displacements(boost::shared_ptr<Molecule>, Options&);
...
std::vector<SharedMatrix> py_psi_mixed_atomic_displacements(boost::shared_ptr<Molecule> mol)
{
  py_psi_prepare_options_for_module("FINDIF");
  return findif::mixed_atomic_displacements(mol, Process::environment.options);
}
std::vector<SharedMatrix> py_psi_normal_mode_displacement_vectors(
    boost::shared_ptr<Molecule>mol,SharedMatrix F)
{
  py_psi_prepare_options_for_module("FINDIF");
  return findif::normal_mode_displacement_vectors(mol,Process::environment.options,F);
}

// Converts a C++ std::pair to a python tuple
template<class K, class V>
boost::python::tuple toPythonTuple(std::pair<K, V> pair)
{
  return boost::python::make_tuple(pair.first,pair.second);
}
boost::python::list py_psi_normal_mode_rms_amp_displacements(
    boost::shared_ptr<Molecule>mol,SharedMatrix F)
{
  std::vector<std::pair<SharedMatrix, double>>
    disp_and_amps_vector  = findif::normal_mode_rms_amp_displacements(
      mol,Process::environment.options,F);
  boost::python::list disp_amps_list;
  for(auto &disp_amp_pair: disp_and_amps_vector){
    disp_amps_list.append(toPythonTuple(disp_amp_pair));
  }
  return disp_amps_list;
}
...
//    def("mol_from_file",&LibBabel::ParseFile,"Reads a molecule from another input file");
    ...
    def("atomic_displacements", py_psi_atomic_displacements,
        "Returns list of displacements generated by displacing each atom in the +/- x, y, z directions");
    def("mixed_atomic_displacements", py_psi_mixed_atomic_displacements,
        "Returns a list of mixed atomic displacements [m][n] for coordinate [m] every [n] >= [m] is displaced +/-");
    def("normal_mode_displacement_vectors",
        py_psi_normal_mode_displacement_vectors,
        "Returns a list of displacement vectors(psi4::Matrix) along normal "
        "modes.\n"
        "\tNOTE: The 3n length displacement vector is in the shape of a "
        "geometry matrix, natom x 3, so new geometries can be created by "
        "geom +/- displacment");
    def("normal_mode_rms_amp_displacements",
        py_psi_normal_mode_rms_amp_displacements,
        "Returns a list of tuples (displacement, size) where "
        "displacement is a vector (psi4::Matrix) corresponding to displacement "
        "along a normal mode, and size is the displacement size according "
        "to the rms amplitude of that vibration at the temperature requested. \n"
        "\tNOTE: The 3n length displacement vector is in the shape of a "
        "geometry matrix (natom x 3), so new geometries can be created by "
        "geom +/- displacement.scale");
